---
title: "Attested Secure Channel Establishment: Use Cases and Requirements"
...
(D)TLS v1.3 Handshake protocol. Traditional peer authentication in TLS
establishes trust in a peer's network identifiers but provides no assurance
regarding the integrity of its underlying software and hardware stack. Remote
attestation addresses this gap by enabling a peer to provide verifiable evidence
about its current state, including the state of its trusted computing base
(TCB). This document specifies a set of essential properties the protocol
solution must have, including cryptographic binding to the TLS connection,
evidence freshness, and flexibility to support different attestation models. It
also explores the use cases which motivate the need for these properties, such
as confidential data collaboration and secure secrets provisioning. This
document is intended to serve as an input to the design of protocol solutions
...
* TLS: The (Datagram) Transport Layer Security protocol. Throughout this
  document, unless specified otherwise, it refers to both TLS and DTLS.
...
These properties are necessary for any valid protocol design, regardless of any
use-cases they might be intended for.

### Attestation Credential binding to the Secure Channel

Attestation Evidence or Attestation Results are cryptographically bound to the
specific secure channel instance (e.g., the TLS connection). This prevents
replay and relay attacks where an attacker presents valid but old or unrelated
attestation credentials from a different connection or context. This binding is
paramount for all use cases.

### Attestation Credential Freshness

The Relying Party can verify that the Evidence or Attestation Result it receives
was freshly generated by the Attester. Since the attester's security state can
change at any moment, stale evidence or attestation results might no longer
present relevant insights. See {{Section 10 of -rats-arch}} for more details
about freshness in the context of RA.
...
for deployments requiring maximum freshness, while the Passport model is better
suited for performance, scalability, and scenarios where the Verifier may
introduce additional latency, or may be offline or unreachable by the Relying
Party.

### Integration with Authentication

The solution should support using RA in conjunction with traditional PKI-based
authentication (e.g., X.509 certificates). This provides two independent pillars
of trust: trustworthiness (from RA) and network identity (from PKI).

### Privacy Preservation

The solution must limit its effect on the privacy properties of the secure
channel establishment protocol. Evidence can contain highly specific, unique
information about a device's hardware and software, which could be used as an
advanced tracking mechanism. The design must consider how to minimize this
leakage.

### Performance and Efficiency

The introduction of attestation should not add prohibitive latency or overhead
to the connection establishment process. Multiple additional round-trips or very
large payloads in the initial handshake should be minimized so that the
solution remains practical.

## Use-Case-Specific Properties

These properties are only required for specific use-cases.
...
Evidence collected at certificate issuance or during the initial secure channel
establishment reflects only the Target Environment's state at that moment. It
cannot guarantee that the Target Environment remains trustworthy for the
lifetime of the certificate or even for the duration of the TLS session. Runtime
attestation closes this gap by enabling the Relying Party to request new
attestation evidence once the TLS connection has been established, or
periodically during long-lived connections if necessary. This allows the Relying
Party to detect posture changes that affect trustworthiness.
...
guarantees needed from RA are described. The Generic Integration Properties
apply to all cases; each use case notes any additional use-case-specific
properties from the previous section.
...
Relevant properties: Generic Integration Properties; Runtime Attestation (for
...
Relevant properties: Generic Integration Properties; Cryptographic Binding to
Machine Identifier; Runtime Attestation (for device lifecycle changes).
...
Relevant properties: Generic Integration Properties; Cryptographic Binding to
Machine Identifier; Runtime Attestation.
