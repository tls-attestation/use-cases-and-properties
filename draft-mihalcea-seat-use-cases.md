...
Evidence from a different connection or context. This binding is paramount for
all use cases.

### Attestation Credential Freshness

The Relying Party can verify that the Evidence or Attestation Result it receives
was freshly generated by the Attester for the current connection. State is
transient, and credentials from a previous connection may no longer be valid.
See {{Section 10 of -rats-arch}} for more details about freshness in the context
of RA.
...
leakage, especially when a third-party Verifier is involved in the protocol
exchange.
...
Evidence is cryptographically bound to an identifier provided to the machine by
the infrastructure provider to prevent diversion attacks {{Meeting-122-TLS-Slides}}.
This property is especially important in confidential computing scenarios where
